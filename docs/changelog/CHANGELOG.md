...
**Garantia A (commit `0f0696a`, deploy confirmado ao vivo em produção):** `listTours()`/`getTour()` (`src/data/sources/nauticflow-source.ts`) trocaram `next: { revalidate: 300 }` por `cache: 'no-store'`. `listDestinations()`/`listCategories()` continuam com cache de 300s (a constante foi renomeada de `CONTENT_REVALIDATE_SECONDS` para `TAXONOMY_REVALIDATE_SECONDS`, mais precisa). Isso já torna `/`, `/passeios`, `/passeios/[destino]` e `/destinos` dinamicamente renderizadas por requisição, sem precisar de `force-dynamic` explícito em nenhuma delas. No processo, `next build` quebrou de um jeito revelador: o `catch` genérico de `fetchJson()` embrulhava QUALQUER erro (inclusive o sinal interno do Next.js `digest: 'DYNAMIC_SERVER_USAGE'`, usado internamente pra bailar uma rota de renderização estática) como `DataSourceError`, convertendo um bailout normal num erro de build de verdade — corrigido deixando esse erro específico atravessar sem ser embrulhado. Confirmado ao vivo: `curl -I https://toursflow.com.br/` mostra `Cache-Control: private, no-cache, no-store, max-age=0, must-revalidate` e `age: 0` depois do deploy propagar (o deploy anterior, ISR, mostrava `age: 1611`/`public, max-age=0, must-revalidate`).

**Garantia B (mesmo commit, requer `NEXT_PUBLIC_NAUTICFLOW_SUPABASE_URL`/`_ANON_KEY` em Production — pendência formal registrada em DECISIONS.md):** novo componente client global `CatalogRefresh` (`src/components/realtime/CatalogRefresh.tsx`, montado no `RootLayout`) assina `UPDATE` via Supabase Realtime (Postgres Changes) numa tabela mínima do NauticFlow (`public.marketplace_catalog_state` — só `id`/`version`/`updated_at`, sem dado de passeio/operador/cliente/pagamento) e chama `router.refresh()` com debounce de 400ms quando a versão muda. Nova dependência `@supabase/supabase-js`. As duas env vars NÃO são segredos novos — são os mesmos dois valores já públicos no bundle do navegador do próprio NauticFlow.

Antes de escrever o componente, a arquitetura inicialmente proposta (Broadcast privado com Authorization pra `anon`) foi descartada por exigir cliente autenticado (o visitante do ToursFlow nunca tem sessão no Supabase do NauticFlow) — trocada por Postgres Changes sobre a tabela singleton, e só implementada depois de uma prova real (script Node com só a anon key, sem sessão, recebendo o evento de verdade contra o Supabase de staging do NauticFlow).

`npm run typecheck`/`lint`/`test` (303 testes) limpos, `next build` sucesso. Smoke local (`next start` com as env vars de produção do NauticFlow) confirmou a página carregando sem erro de servidor e o componente presente no HTML renderizado.

**Não verificado a partir desta sessão** (sem ferramenta de navegador disponível): a observação literal de "abrir uma aba, publicar no NauticFlow, ver atualizar sozinha" — a entrega de evento real pra um cliente `anon` foi provada via script Node (WebSocket real, não simulado), e a lógica de bump nos triggers do NauticFlow foi provada via SQL real (ver DOCUMENTACAO.md do NauticFlow), mas a ponta final (React reagindo no navegador) depende de verificação manual do usuário ou de uma sessão com ferramenta de browser.
