...
Data: 2026-08-27 (última atualização: 2026-09-02)
Status: **código publicado em produção (`main`, `217c5bc`), mas o fluxo transacional continua inatingível ao público.** `BookingReview` sabe chamar `POST /api/bookings` de verdade (`src/lib/booking-submission.ts`, cria hold real no NauticFlow) — só quando `BOOKING_CHECKOUT_ENABLED` está ligada (hoje `false`, ADR-013). Sem ela, mostra o aviso "fale com o operador", sem botão funcional, e a própria rota falha fechada por conta própria (`422 BOOKING_CHECKOUT_NOT_ENABLED`, confirmado ao vivo contra `https://toursflow.com.br`, sem chamada upstream). Infraestrutura validada em E2E real anterior (caminho de sucesso, replay e conflito de idempotência confirmados contra produção); rate limit por visitante implementado e testado; rota reforçada (Content-Type, limite de corpo real, Origin/Sec-Fetch-Site). Nenhum E2E controlado do fluxo completo pela UI foi executado contra produção com a flag ligada (sem mecanismo de cleanup — ver [DECISIONS.md](DECISIONS.md), ADR-009); validação é por testes automatizados + verificação em browser real (Playwright, inclusive contra produção) até o step de revisão.
...
- **`BookingSelector`/`BookingReview` sabem chamar `/api/bookings` de verdade** (seleção → `CustomerForm` → `BookingReview` → `BookingConfirmation`) — payload whitelisted e normalizado (`src/lib/booking-submission.ts`), `Idempotency-Key` com ciclo de vida completo (`resolveIdempotencyKey()`), todos os `BookingErrorCode` tratados com mensagem segura, double-submit impedido, countdown do hold baseado em `holdExpiresAt` do servidor. **Atrás de `BOOKING_CHECKOUT_ENABLED = false`** (`src/lib/feature-flags.ts`) — enquanto desligada, `BookingReview` não recebe `onConfirm` e mostra o mesmo aviso "fale com o operador" de antes desta fase, e a própria rota `POST /api/bookings` falha fechada por conta própria (`422 BOOKING_CHECKOUT_NOT_ENABLED`, antes de Origin/Content-Type/parsing/qualquer chamada ao NauticFlow) — não depende da ausência do botão. Ver [ADR-013](DECISIONS.md#adr-013--booking-rollout-gate-booking_checkout_enabled). Validado por testes automatizados (mockados e, à parte, sem mock, exercitando o valor real da flag) e verificação em browser real até o step de revisão.
- **Checkout/pagamento Pix também já existe, mais adiante na mesma branch local.** Contrato real do NauticFlow confirmado e integrado ponta a ponta (rota `/api/bookings/[bookingId]/payment`, `ToursFlowPaymentClient`, `PixPayment`/`BookingVoucher`) — ver [PAYMENTS.md](PAYMENTS.md), [DECISIONS.md](DECISIONS.md) ADR-011/ADR-012. Continua atrás de `PAYMENTS_UI_ENABLED = false` — que, além de tudo, é logicamente posterior a `BOOKING_CHECKOUT_ENABLED` (não existe pagamento sem reserva primeiro). Nenhuma das duas publicada.
...
4. **Checkout/pagamento (Pix) já está implementado ponta a ponta e publicado em produção** (contrato real confirmado, wiring completo, testado — ver [PAYMENTS.md](PAYMENTS.md), ADR-011/ADR-012 em [DECISIONS.md](DECISIONS.md)), mas continua atrás de `PAYMENTS_UI_ENABLED = false` — fluxo transacional em si não publicado. Ligar a flag exige `MARKETPLACE_PAYMENTS_ENABLED` em produção no NauticFlow **e** revisão explícita autorizando a mudança — as duas condições, não uma ou outra.
5. **A infraestrutura de reserva/hold em si também já está pronta e testada**, mas atrás de `BOOKING_CHECKOUT_ENABLED = false` (ADR-013) — decisão deliberada de publicar o código sem publicar o fluxo transacional, para separar "deploy técnico" de "prontidão operacional" (equipe pronta para acompanhar/confirmar holds manualmente, como a própria UI promete ao turista). Ligar `BOOKING_CHECKOUT_ENABLED` é uma decisão de negócio, não técnica — a infraestrutura não precisa de nenhum trabalho adicional para isso.
